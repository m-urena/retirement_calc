...
# End value labels
fig.add_annotation(x=ages[-1], y=baseline[-1],
                   text=f"${baseline[-1]:,.0f}", showarrow=False,
                   font=dict(color="#7D7D7D", size=13, family="Segoe UI"),
                   xanchor="left", yanchor="middle", xshift=10)
fig.add_annotation(x=ages[-1], y=with_help[-1],
                   text=f"${with_help[-1]:,.0f}", showarrow=False,
...
fig.update_layout(
    ...
    hovermode="x unified", margin=dict(r=80, t=60, l=60, b=60)
)
